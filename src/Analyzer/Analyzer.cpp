...
void Analyzer::susceptibility(Analyzer &analyzer, int amount_of_sample_sets, const std::string &observable_name) {
    if (blocked_data.empty()) {
        if (int_auto_correlation_time > 0) {
            block_data(static_cast<int>(int_auto_correlation_time + 1) / 2, -1);
        } else {
    ...
    if(analyzer.blocked_data.empty()){
        if (analyzer.int_auto_correlation_time > 0) {
            analyzer.block_data(static_cast<int>(analyzer.int_auto_correlation_time + 1) / 2, -1);
        } else {
            analyzer.block_data(42, -1);
        }
    }
    if (blocked_data.empty()) {
        return;
    ...
    std::vector<double> susceptibility_bootstrapped_data;
    susceptibility_bootstrapped_data.resize(amount_of_sample_sets);
    std::fill(susceptibility_bootstrapped_data.begin(), susceptibility_bootstrapped_data.end(), 0.);
